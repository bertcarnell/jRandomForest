 ...
 * 
 * rfutils.cs is based on rfutils.c from the R package randomForest v4.6-6 licensed under the GPL v2 or later
...
namespace CsRandomForest
{
    ...
    public class rfutils
    {
        ...
        /// <param name="length">the length of the locations to fill (not necessarily the full length)</param>
        public static void zeroInt(ref int[] x, int length)
        {
            Debug.Assert(x.Length >= length);
        ...
        /// <param name="x">the array to fill</param>
        /// <param name="length">the length of the locations to fill (not necessarily the full length)</param>
        public static void zeroInt(CArray<int> x, int length)
        {
            Debug.Assert(x.Length >= length);
            for (int i = 0; i < length; i++)
                x[i] = 0;
        ...
        /// <param name="x">the array to fill</param>
        /// <param name="length">the length of the locations to fill (not necessarily the full length)</param>
        public static void zeroDouble(ref double[] x, int length)
        {
            Debug.Assert(x.Length >= length);
            for (int i = 0; i < length; i++)
                x[i] = 0.0;
        ...
        /// <param name="x">the array to fill</param>
        /// <param name="length">the length of the locations to fill (not necessarily the full length)</param>
        public static void zeroDouble(CArray<double> x, int length)
        {
            Debug.Assert(x.Length >= length);
            for (int i = 0; i < length; i++)
                x[i] = 0.0;
        ...
        public static void normClassWt(CArray<int> cl, int nsample, int nclass,
                 int useWt, ref double[] classwt, ref int[] classFreq) {
            int i;
            double sumwt = 0.0;
            ...
            for (i = 0; i < nclass; ++i) {
                classwt[i] = (classFreq[i] > 0) ? classwt[i] * nsample / classFreq[i] : 0.0;
            }
        }

        /// <summary>
        /// makeA() constructs the <code>mdim by nsample</code> integer array a.  For each 
        ...
        /// <param name="b"></param>
        public static void makeA(ref double[] x, int mdim, int nsample, CArray<int> cat, ref int[] a,
           CArray<int> b) {
            int i, j, n1, n2;
            int[] index;
            double[] v;
            ...
            for (i = 0; i < mdim; ++i) {
                if (cat[i] == 1) { /* numerical predictor */
                    ...
                    Array.Sort(v, index); // sort the items (index) by the keys (v) and the keys (v)
        ...
        public static void createClass(ref double[] x, int realN, int totalN, int mdim, Random oRandom)
        {
            ...
            for (i = realN; i < totalN; ++i)
            {
                for (j = 0; j < mdim; ++j)
                {
                    ...
                    k = Convert.ToInt32(Math.Floor(oRandom.NextDouble() * Convert.ToDouble(realN)));
        ...
        public static void modA(CArray<int> a, ref int nuse, int nsample, int mdim,
	          CArray<int> cat, ref int maxcat, CArray<int> ncase, ref int[] jin) {
            int i, j, k, m, nt;
            ...
            for (i = 0; i < mdim; ++i) {
              ...
              if (cat[i] == 1) {
                  for (j = 0; j < nsample; ++j) {
                      if (jin[a[i + k * mdim] - 1] > 0) {
                          a[i + nt * mdim] = a[i + k * mdim];
                          k++;
                      } else {
                          for (m = 0; m < nsample - k; ++m) {
                              if (jin[a[i + (k + m) * mdim] - 1] > 0) {
        ...
        public static void Xtranslate(double[] x, int mdim, int nrnodes, int nsample,
            CArray<int> bestvar, CArray<int> bestsplit, CArray<int> bestsplitnext,
            CArray<double> xbestsplit, CArray<int> nodestatus, CArray<int> cat, int treeSize)
        {
            int i, m;
            ...
            for (i = 0; i < treeSize; ++i)
            {
                if (nodestatus[i] == 1)
                {
                    ...
                    {
                        xbestsplit[i] = Convert.ToDouble(bestsplit[i]);
                    }
                }
            }
        }
        ...
        /* Compute proximity. */
        public static void computeProximity(ref double[] prox, int oobprox, CArray<int> node, 
            ref int[] inbag, ref int[] oobpair, int n)
        {
        ...
        /// <param name="oRandom">Random number generator</param>
        public static void permuteOOB(int m, ref double[] x, ref int[] _in, int nsample, int mdim, Random oRandom) {
            double tmp;
            double[] tp;
            int i, last, k, nOOB = 0;
        ...
        #else
		        k = (int) ((double)last * unif_rand());
        #endif*/
                k = Convert.ToInt32(Math.Floor(Convert.ToDouble(last) * oRandom.NextDouble()));
		        tmp = tp[last - 1];
		        tp[last - 1] = tp[k];
		        tp[k] = tmp;
		        last--;
        ...
        /// <returns></returns>
        public static uint pack(int nBits, ref int[] bits) {
            int i = nBits;
	        uint pack = 0;
            while (--i >= 0) pack += Convert.ToUInt32(bits[i] << i);
            return pack;
        }

        /// <summary>
        /// The subroutine returns icat, an integer array of zeroes and ones corresponding to the coefficients 
        /// in the binary expansion of pack
        /// </summary>
        /// <param name="nBits"></param>
        /// <param name="pack">4-byte integer</param>
        /// <param name="bits"></param>
        public static void unpack(int nBits, uint pack, ref int[] bits) {
            int i;
            for (i = 0; i < nBits; pack >>= 1, ++i) bits[i] = Convert.ToInt32(pack & 1);
        }
